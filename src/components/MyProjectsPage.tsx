...
export function MyProjectsPage({ onNavigateHome }: MyProjectsPageProps) {
  ...
  const formatCoordinates = (coords?: string) => {
    ...
    try {
      // Parse the string "[lon, lat]" format from Directus
  ...
  return (
    <div className="my-projects-page">
      {/* Header - positioned below dropdown menu */}
      <header className="projects-header">
        <h1>
          <ProjectIcon />
          My Projects
        </h1>
        <p className="subtitle">Projects you have interacted with in Forma</p>
      </header>
      ...
      <main className="projects-content">
        {isLoading ? (
          ...
          <div className="empty-state">
            ...
            <button onClick={onNavigateHome} className="primary-button">
              Go to Home
            </button>
          </div>
        ) : (
